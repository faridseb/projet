...
include "connect.php";
    ...
    if(isset($_POST['ok'])){
        ...
        if(!empty($nom) AND !empty($prenom) AND !empty($email) AND !empty($mdp) AND !empty($role) ){
                        if($nom == " " or $prenom == " "){
                            $erreur = "LE CHAMP DOIT CONTENIR DES CARACTERE" ;
                        }
                        else{
                            $reqEmail = $bdd->prepare("SELECT * FROM utilisateur WHERE email_utilisateur = ?");
                        $reqEmail->execute(
                            array($email)
                        );
                        if($reqEmail->rowCount() == 0 ){
                                if(strlen($_POST['mdp']) < 4){
                                    ...
                                    $requete = $bdd->prepare("INSERT INTO utilisateur(nom_utilisateur,prenom_utilisateur,email_utilisateur,mdp_utilisateur,id_role) VALUES (?,?,?,?,?)");
                                    $requete->execute(
        ...
        <form action="" method="POST">
            ...
            <div class="box">
                ...
                <select name="role" id="" required>
                    <option value="1">DEVELOPPEUR FRONT-END</option>
                    <option value="2">DEVELOPPEUR BACK-END</option>
                    <option value="3">DEVELOPPEUR FULL-STACK</option>
                </select>
            </div>
            <div class="box">
